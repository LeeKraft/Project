...
class CollectorWindow:
    ...
    def __init__(self):
        ...
        # frame
        self.frm_top = LabelFrame(master=self.root, width=600, height=320, relief=SUNKEN, borderwidth=5, bg="#e9ffe3", font=("Arial",12, "bold"), text="Параметры")
        self.frm_middle = Frame(master=self.root, width=600, height=50, relief=SUNKEN, borderwidth=5, bg="#b4d4ab")
        self.frm_bottom = LabelFrame(master=self.root, width=600, height=130, relief=SUNKEN, borderwidth=5, bg="#e9ffe3", font=("Arial",12, "bold"), text="Статус")
        # label
        self.lbl_inter = Label(master=self.frm_top, text="Интерфейс", font=("Arial",14), bg="#e9ffe3")
        self.lbl_inter.place(relx=0.07, rely=0.07)
        self.lbl_host = Label(master=self.frm_top, text="IP адрес", font=("Arial",14), bg="#e9ffe3")
        self.lbl_host.place(relx=0.07, rely=0.19)
        self.lbl_port = Label(master=self.frm_top, text="Порт", font=("Arial",13), bg="#e9ffe3")
        self.lbl_port.place(relx=0.71, rely=0.19)
        self.lbl_src = Label(master=self.frm_top, text="IP адрес источника", font=("Arial",14), bg="#e9ffe3")
        self.lbl_src.place(relx=0.07, rely=0.31)
        self.lbl_src_port = Label(master=self.frm_top, text="Порт", font=("Arial",14), bg="#e9ffe3")
        self.lbl_src_port.place(relx=0.71, rely=0.31)
        self.lbl_dst = Label(master=self.frm_top, text="IP адрес назначения", font=("Arial",14), bg="#e9ffe3")
        self.lbl_dst.place(relx=0.07, rely=0.43)
        self.lbl_dst_port = Label(master=self.frm_top, text="Порт", font=("Arial",14), bg="#e9ffe3")
        self.lbl_dst_port.place(relx=0.71, rely=0.43)
        self.lbl_proto = Label(master=self.frm_top, text="Протокол", font=("Arial",14), bg="#e9ffe3")
        self.lbl_proto.place(relx=0.07, rely=0.55)
        self.lbl_time = Label(master=self.frm_top, text="Время сбора пакетов", font=("Arial",14), bg="#e9ffe3")
        self.lbl_time.place(relx=0.07, rely=0.67)
        self.lbl_time_unit = Label(master=self.frm_top, text="(s/m/h/d)", font=("Arial",14), bg="#e9ffe3")
        self.lbl_time_unit.place(relx=0.7, rely=0.67)
        self.lbl_count = Label(master=self.frm_top, text="Количество пакетов", font=("Arial",14), bg="#e9ffe3")
        self.lbl_count.place(relx=0.07, rely=0.79)
        # radiobutton
        self.choice_addr = IntVar(value=0)
        self.choice_time_count = IntVar(value=0)
        self.rbtn_host = Radiobutton(master=self.frm_top, variable=self.choice_addr, value=0, bg="#e9ffe3", activebackground="#b4d4ab", command=self.change_host)
        self.rbtn_host.place(relx=0.01, rely=0.19)
        self.rbtn_src_dst = Radiobutton(master=self.frm_top, variable=self.choice_addr, value=1, bg="#e9ffe3", activebackground="#b4d4ab", command=self.change_host)
        self.rbtn_src_dst.place(relx=0.01, rely=0.31)
        self.rbtn_time = Radiobutton(master=self.frm_top, variable=self.choice_time_count, value=0, bg="#e9ffe3", activebackground="#b4d4ab", command=self.change_mode)
        self.rbtn_time.place(relx=0.01, rely=0.67)
        self.rbtn_count = Radiobutton(master=self.frm_top, variable=self.choice_time_count, value=1, bg="#e9ffe3", activebackground="#b4d4ab", command=self.change_mode)
